...
CtrlActionGroupTable.exec = function () {
    ...
    let list = this._buildGroupsListHTML(this, this.groups);
 ...
 */
CtrlActionGroupTable._buildGroupsListHTML = function (_this, groups) {
...
CtrlActionGroupTable.getGroupDevices = function (e) {
    ...
    if (response && response.ResponseMessage && response.ResponseMessage.Reply && response.ResponseMessage.Reply.Result
        ...
        const prefix = response.ResponseMessage.Payload.DeviceGroup.DeviceGroup._Prefix ? response.ResponseMessage.Payload.DeviceGroup.DeviceGroup._Prefix : "---";
        if (groups) {
            if (groups.length === undefined)
                groups = [groups];
...
CtrlActionGroupTable.editGroup = function (e) {
    ...
    if (response && response.ResponseMessage && response.ResponseMessage.Reply && response.ResponseMessage.Reply.Result
        ...
        const selectedGroupPrefix = response.ResponseMessage.Payload.DeviceGroup.DeviceGroup["_Prefix"] ?
            response.ResponseMessage.Payload.DeviceGroup.DeviceGroup["_Prefix"] : "";
        let devArr = [];
...
CtrlActionGroupTable.addGroup = function (group, prefix, devices) {
    ...
    $.confirm({
        title: group ? AppMain.t("EDIT_GROUP", "GROUP_TABLE") : AppMain.t("ADD_GROUP", "GROUP_TABLE"),
        content: allHtml,
        useBootstrap: false,
        draggable: false,
        theme: "material",
        buttons: {
            confirm: {
                text: group ? AppMain.t("OK", "global") : AppMain.t("CREATE", "global"),
                action: function () {
                    let groupObj = {};
                    if (group) {
    ...
    setTimeout(function () {
        ...
        let prefix =  $("input[name='group-prefix']");
        if(prefix.val() !== ""){
            $(".dynamic-hide").hide();
        }
...
CtrlActionGroupTable.getRestJson = function(groupObj){
    ...
    let obj = {
        "mes:Header": {
            "mes:Verb": groupObj.isEdit ? "change" : "create",
            "mes:Noun": "DeviceGroup",
            "mes:Timestamp": moment().toISOString(),
            "mes:MessageID": "78465521",
            "mes:CorrelationID": "78465521",
...
CtrlActionGroupTable.addGroupRest = function (groupObj) {
    ...
    if (response && response.ResponseMessage && response.ResponseMessage.Reply && response.ResponseMessage.Reply.Result
        && response.ResponseMessage.Reply.Result.toString() === "OK") {
        groupObj.isEdit ? AppMain.dialog("GROUP_UPDATED", "success") : AppMain.dialog("GROUP_CREATED", "success");
        this.exec();
    }
    return true;
};
