...
CtrlActionMonitoringManager.initDatepickers = function(){
    ...
    $(this.selectorFrom).on("change", function(){
...
CtrlActionMonitoringManager.drawGraph = function(){
    ...
    var duration = moment.duration(toD.diff(fromD));
...
CtrlActionMonitoringManager.drawGraphSetUp = function(){
    ...
    const color = defined(this.categoryToColor[profileCategory]) ? this.categoryToColor[profileCategory] : "rgb(255, 172, 171)";

    profileCategory = profileCategory.toLowerCase();
    if(profileCategory.indexOf(("CNT-WAN").toLowerCase()) !== -1){
        profileCategory = "CNT-WAN".toLowerCase();
    }
    ...
    let chartType = this.typeObj[profileType] === "COUNTER" ? "line" : "bar";
    if(!this.firstTime) {
        this.chart.destroy();
    }
    this.firstTime = false;

    const units = defined(this.unitsObj[profileType]) ? " (" + this.unitsObj[profileType] + ")": "";
...
CtrlActionMonitoringManager.setCounters = function(counters,profileCategory,profileType ){
    ...
    for(let i = start; i < counters.length; i++){
...
CtrlActionMonitoringManager.export = function(){
    ...
    for(let i = 0; i < this.chartLabels.length; i++){
        csv += '"' + this.chartLabels[i] + '"' +", ";
        csv += this.chartDatasets[0].data[i] + "\r\n";
    }
