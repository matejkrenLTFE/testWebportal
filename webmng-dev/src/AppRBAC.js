...
module.exports.AppRBAC = function () {
    ...
    this.processViewTemplate = function (content) {
        ...
        $.each(rbacWrapperSelectors, function (i, element) {
        ...
        $.each(rbacElementSelectors, function (i, element) {
            const $element = $(element);
    ...
    this.getCtrlActionRBACName = function (actionName) {
        ...
        return (defined(map[actionName])) ? map[actionName] : actionName.toLowerCase();
    };
    ...
    this.hasExecCtrlActionPermission = function (actionName) {
        ...
        if (userRoleData) {
            for (let i in userRoleData.mainmenu) {
                if(userRoleData.mainmenu.hasOwnProperty(i)){
                    if (i === ctrlActionName && userRoleData.mainmenu[i] === "h") {
                        allowExecute = false;
                        break;
                    }
                }
