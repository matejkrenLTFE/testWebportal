...
CtrlActionEvents.exec = function() {
    ...
    this.view.renderEmpty("Events", {
        _title: AppMain.t("EVENT_LOGS", "EVENTS"),
        events: events,
    ...
    if (defined(response) && defined(response.event) && response.event instanceof Array)
        events.data = response.event;
    else if (defined(response) && defined(response.event) && defined(response.event.id))
        events.data = [response.event];
    else
        events.data = [];
    ...
    if (events.data.length > 0) {
        $.each(events.data, function(i, item){
            ...
            events.htmlList += "<td class='system-title' style=\"text-align: left!important;\">" + (defined(item["system-title"]) ? item["system-title"] : "---") + "</td>";
            events.htmlList += "<td class='mac' style=\"text-align: left!important;\">" + (defined(item["mac-address"]) ? item["mac-address"] : "---") + "</td>";
            // events.htmlList += "<td class='ip' style=\"text-align: left!important;\">" + (defined(item["ip-address"]) ? item["ip-address"] : "---") + "</td>";
            events.htmlList += "<td class='ip' style=\"text-align: left!important;\">"+ (defined(item["ip-address"]) ? item["ip-address"]
                + " " : "") + (defined(item["info-message"]) ? item["info-message"] : "---") + "</td>";
            events.htmlList += "</tr>";
        });
    }
    else {
    ...
    this.view.render("Events", {
        ...
        formApply: AppMain.t("APPLY", "global"),
        formSelectType: AppMain.t("EVT_LOG_TYPE", "EVENTS"),
        totalResults: defined(events.data.length) ? AppMain.t("SHOWING_TOTAL", "EVENTS") + "<span id='total-count'>" +events.data.length +"</span>" : "",
...
CtrlActionEvents.__exportEventsList = function() {
    ...
    if (CtrlActionEvents.eventsData.length > 0) {
        ...
        $(CtrlActionEvents.eventsData).each(function(i, evt){
            csv += '"' + (defined(evt.id) ? AppMain.t(evt.id, "EVENTS") : "") + '",';
            csv += '"' + (defined(evt.id) ? CtrlActionEvents.eventListIDMap[evt.id] : "") + '",';
            csv += '"' + (defined(evt.id) ? AppMain.t(CtrlActionEvents.eventListMap[evt.id], "EVENTS") : "") + '",';
            csv += '"' + (defined(evt["time-stamp-iso"]) ? evt["time-stamp-iso"] : "") + '",';
            csv += '"' + (defined(evt["system-title"]) ? evt["system-title"] : "") + '",';
            csv += '"' + (defined(evt["mac-address"]) ? evt["mac-address"] : "") + '",';
            csv += '"' + (defined(evt["ip-address"]) ? evt["ip-address"] + " " : "") + (defined(evt["info-message"]) ? evt["info-message"] : "---") + '"';
            csv += '\r\n';
        });
